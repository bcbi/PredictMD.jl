...
elseif have_pdflatex
    ...
    warn(string("WARN No LaTeX installation found, figures will not be generated. ",
...
if !have_pdftoppm
    warn(string("WARN Did not find `pdftoppm`, png output will be disabled. Install `pdftoppm` ",
...
if !have_pdf2svg
    warn(string("WARN Did not find `pdf2svg`, svg output will be disabled. Install `pdf2svg` ",
        "and run Pkg.build(\"PredictMD\") to enable"))
end

if !have_pdf2svg && !have_pdftoppm
    warn(string("WARN Found neither pdf2svg or pdftoppm, figures will not be viewable in Jupyter or Juno"))
end
