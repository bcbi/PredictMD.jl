...
Test.@test(
    strip(metrics[r2_row, Symbol("Linear regression")]) > 0.550
    )
Test.@test(
    strip(metrics[r2_row, Symbol("Random forest")]) > 0.550
    )
Test.@test(
    strip(metrics[r2_row, Symbol("Knet MLP")]) > 0.300
    )
mse_row = first(
    findall(
...
Test.@test(
    strip(metrics[mse_row, Symbol("Linear regression")]) < 40.000
    )
Test.@test(
    strip(metrics[mse_row, Symbol("Random forest")]) < 40.000
    )
Test.@test(
    strip(metrics[mse_row, Symbol("Knet MLP")]) < 65.000
    )
rmse_row = first(
    findall(
...
Test.@test(
    strip(metrics[rmse_row, Symbol("Linear regression")]) < 6.500
    )
Test.@test(
    strip(metrics[rmse_row, Symbol("Random forest")]) < 6.500
    )
Test.@test(
    strip(metrics[rmse_row, Symbol("Knet MLP")]) < 8.000
    )
# PREDICTMD ELSE
# PREDICTMD ENDIF INCLUDE TEST STATEMENTS
